<template>
  ...
  <el-row v-for="(volume, vidx) in volumeData" :index="volume.volumeid">
    ...
    <el-col style="line-height:25px;padding:10px; border-bottom: 1px solid #ddd;" :span="8"
      v-for="(chapter, index) in chapterData[volume.id]" :index="chapter.id">
...
const getBook = () => {
  BookService.getBook({ id: bookid }).then(
    resp => {
      for (var i in resp.data) {
        currentBook[i] = resp.data[i]
      }
