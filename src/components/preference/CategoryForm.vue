<template>
  <div class="flex gap-2" style="flex-wrap: wrap;">
    <el-tag v-for="cat, id in gostore.categories" :key="id" type="info" effect="plain" closable :disable-transitions="false" @close="handleDelete(cat, id)" @click="handleEditCategory(cat, id)"
      style="cursor: pointer;"> {{ cat.title }} ({{ cat.bookcount }}) </el-tag>
    <el-input v-if="newCatVisible" ref="newCatRef" v-model="newCatVal" class="w-20" size="small" @keyup.enter="saveNewCategory" @blur="saveNewCategory" />
    <el-button v-else size="small" icon="IconoirGridAdd" @click="handleNewCategoryInput">{{ $t('config.btn_new_category') }} </el-button>
  </div>
...
const handleEditCategory = (cat: Category, idx: number) => {
  ElMessageBox.prompt(_t('config.category_edition'), {
    ...
    inputValidator: (val) => { return val ? true : _t('config.category_edition_title_is_empy') },
  })
    .then(({ value }) => {
