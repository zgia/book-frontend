<template>
  ...
  <el-table v-loading="fsLoading" :header-row-class-name="tableHeadClass" :data="bookData" :highlight-current-row="true" tripe border>
    ...
    <el-table-column :label="$t('book.rate')" width="80" v-if="showRate">
      <template #default="scope">
    ...
    <el-table-column :label="$t('book.category')" width="80" v-if="showAuthor">
      <template #default="scope">
    ...
    <el-table-column :label="$t('book.wordcount')" width="80" v-if="showWordcount">
      <template #default="scope">
...
<script lang="ts" setup>
  ...
  const handleChangePage = (p: number) => {
    ...
    console.log('handleChangePage', searchForm)
    router.push({ name: 'booklist', query: searchForm })
  }
  ...
  onMounted(() => {
    const { q, sm = '', p = 1, ob = 'id' } = router.currentRoute.value.query;
