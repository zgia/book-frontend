<template>
  ...
  <el-form class="editchapter" :model="opForm" ref="ruleFormRef" :label-position="lblPos" label-width="auto"
    ...
    <el-form-item :label="$t('chapter.edit_ext_label')">
      <el-col :span="replaSpan">
        <el-input clearable v-model="repla.searchValue" :placeholder="$t('chapter.edit_ext_search_select_placeholder')"
          autocomplete="off">
          <template #append>
            <el-select clearable v-model="selectsv" :placeholder="$t('chapter.edit_ext_search_input_placeholder')"
              @change="selectSv()" style="width: 120px;">
              <el-option v-for="key, item in searchValues" :key="key" :label="key" :value="item" />
            </el-select>
          </template>
        </el-input>
      </el-col>
      <el-col :span="replaSpan" :offset="replaOffset">
        <el-input v-model="repla.replaceValue" clearable
          :placeholder="$t('chapter.edit_ext_replace_select_placeholder')" autocomplete="off">
          <template #append>
            <el-select v-model="selectrv" clearable :placeholder="$t('chapter.edit_ext_replace_input_placeholder')"
              @change="selectRv()" style="width: 120px;">
              <el-option v-for="key, item in replaceValues" :key="key" :label="key" :value="item" />
            </el-select>
          </template>
        </el-input>
      </el-col>
  ...
  <div>
    <el-button-group>
      <el-button :icon="DocumentCopy" type="primary" @click="replaceText()">{{ $t('chapter.btn_replace') }}</el-button>
      <el-button :icon="Finished" type="primary" @click="formatContent()">{{ $t('chapter.btn_format') }}</el-button>
      <el-button :icon="Refresh" type="primary" @click="restoreText()">{{ $t('chapter.btn_restore') }}</el-button>
    </el-button-group>
  </div>
  <div style="padding-top: 10px;">
    <el-button size="large" icon="Check" type="primary" style="width: 256px;" @click="submitForm(ruleFormRef)">{{
...
import { Finished, DocumentCopy, Refresh } from '@element-plus/icons-vue'
...
const pageTitle = chapterid ? i18n.global.t('chapter.pagetitle_edit_chapter') : i18n.global.t('chapter.pagetitle_add_chapter')

// 扩展区域编辑框的长度
const replaSpan = ref(10)
const replaOffset = ref(1)
const lblPos: Ref<'top' | 'right' | 'left'> = ref('left' as const)
watch(() => gostore.clientWidth, (sw) => {
...
const rules = reactive<FormRules>({
  title: [
    { required: true, message: i18n.global.t('chapter.edit_title_is_empty'), trigger: 'blur' },
  ],
  content: [
    { required: true, message: i18n.global.t('chapter.edit_content_is_empty'), trigger: 'blur' },
  ]
...
const submitForm = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate((valid, fields) => {
    if (valid) {
      ...
      console.log('CHAPERR: ' + i18n.global.t('common.validated_error'), fields)
    }
  })
}
...
const update = () => {
  BookService.updateChapter(opForm)
    .then(() => {
      ElMessage.success(i18n.global.t('chapter.saved_ok', { 'title': opForm.title }))
    ...
    .catch(err => {
      ElMessage.error('CHAPERR: ' + i18n.global.t('chapter.saved_error', { 'err': (err.messge || err.msg) }))
    })
}
  ...
  BookService.getChapter({ bookid: bookid, id: chapterid })
    .then(resp => {
      ...
      if (d.current) {
        ...
        originContent.value = d.current.content
      }
