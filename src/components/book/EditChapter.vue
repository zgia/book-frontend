<template>
  ...
  <el-form class="editchapter" :model="opForm" ref="ruleFormRef" :label-position="lblPos" label-width="auto" :rules="rules" size="large">
    ...
    <el-form-item :label="$t('chapter.edit_ext_label')">
      <el-col :span="replaSpan">
        <el-input clearable v-model="repla.searchValue" :placeholder="$t('chapter.edit_ext_search_select_placeholder')" autocomplete="off">
          <template #append>
            <el-select clearable v-model="selectsv" :placeholder="$t('chapter.edit_ext_search_input_placeholder')" @change="selectSv()" style="width: 120px;">
      ...
      <el-col :span="replaSpan" :offset="replaOffset">
        <el-input v-model="repla.replaceValue" clearable :placeholder="$t('chapter.edit_ext_replace_select_placeholder')" autocomplete="off">
          <template #append>
            <el-select v-model="selectrv" clearable :placeholder="$t('chapter.edit_ext_replace_input_placeholder')" @change="selectRv()" style="width: 120px;">
...
const pageTitle = chapterid ? _t('chapter.pagetitle_edit_chapter') : _t('chapter.pagetitle_add_chapter')
...
const rules = reactive<FormRules>({
  title: [
    { required: true, message: _t('chapter.edit_title_is_empty'), trigger: 'blur' },
  ],
  content: [
    { required: true, message: _t('chapter.edit_content_is_empty'), trigger: 'blur' },
  ]
...
const update = () => {
  BookService.updateChapter(opForm)
    ...
    .catch(err => {
      ElMessage.error('CHAPERR: ' + _t('chapter.saved_error', { 'err': (err.messge || err.msg) }))
    })
}
...
const formatContent = () => {
  ...
  // })
  // 方法3
  opForm.content = opForm.content.replaceAll(/\n/g, '').replaceAll(indent, '\n' + indent).replace(/^\n/, '')
}
