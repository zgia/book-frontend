<template>
  ...
  <div class="book-func text-right">
    <el-button-group>
      <el-button icon="IconoirEdit" type="primary" @click="handleEditBook()">{{ $t('book.btn_edit_book') }}</el-button>
      <el-button icon="IconoirMultiplePages" type="primary" @click="handleVolume(bookid)">{{ $t('common.btn_volume') }}</el-button>
      <el-button :icon="IconoirDownloadCircle" type="primary" @click="handleDownload" />
    </el-button-group>
    <el-button-group style="margin-left:5px;">
      <el-button icon="IconoirAddPage" type="primary" @click="handleEdit(bookid)" :disabled="noVolume">{{ $t('common.btn_new_chapter') }}</el-button>
      <el-button icon="IconoirPageEdit" type="primary" @click="editMode = !editMode" />
    </el-button-group>
  </div>
  <h2 class="text-left">{{ $t('common.bookindex') }} <xxsmall v-if="currentBook.isfinished">{{ $t('book.finished_book') }} </xxsmall>
    <xxsmall v-else> {{ $t('chapter.beupto') }} {{ currentBook.latest }}</xxsmall>
  </h2>
  <el-row v-for="(volume, vidx) in volumeData" :index="volume.id" :key="vidx">
    ...
    <el-col class="chapters-list" :xs="24" :sm="12" :md="8" :lg="6" :xl="3" v-for="(chapter, index) in chapterData[volume.id]" :index="chapter.id" :key="index">
      <el-button class="chapter-link" v-if="chapter" @click="handleRead(bookid, chapter.id)">{{ subTitle(chapter.title) }}</el-button>
      <p v-if="editMode">
        <el-button-group>
          <el-button text size="small" icon="IconoirPageEdit" @click="handleEdit(bookid, chapter.id)" />
          <el-button text size="small" icon="IconoirTrash" @click="handleDelete(index, chapter, volume.id)" />
        </el-button-group>
      </p>
    </el-col>
  </el-row>
  ...
  <Suspense v-if="editBookVisible">
    <template #default>
      <editBookDialog :book="currentBook" @hide="editBookVisible = false" @update="afterBookUpdated" @close-loading="closeLoading" />
    </template>
  </Suspense>
</template>
