<template>
  <h1>{{ $t('volume.volume_management') }} <xxsmall>{{ currentBook.title }}</xxsmall>
  </h1>
  <el-row>
    <el-col :span="24" style="text-align: right;padding-bottom: 10px;">
      <el-button type="primary" icon="Plus" @click="handleNew()" class="btn-search">{{ $t('volume.pagetitle_add_volume')
        }}</el-button>
    </el-col>
  </el-row>
  <el-table v-loading="fsLoading" :header-row-class-name="tableHeadClass" :data="tableData"
    :highlight-current-row="true" stripe border>
    <el-table-column prop="id" label="ID" width="100" />
    <el-table-column prop="title" :label="$t('volume.title')" width="300" />
    <el-table-column prop="cover" :label="$t('volume.cover')" width="300">
    ...
    <el-table-column :label="$t('common.operation')" width="100">
      <template #default="scope">
        <el-button-group>
          <el-button text size="small" icon="Edit" @click="handleEdit(scope.$index, scope.row)" />
          <el-button text size="small" icon="Delete" @click="handleDelete(scope.$index, scope.row)" />
        </el-button-group>
      </template>
    </el-table-column>
  </el-table>
  <Suspense v-if="newVolumeVisible">
    <template #default>
      <newVolumeDialog :volume="currentVol" @hide="newVolumeVisible = false" @update="afterUpdated"
...
// ***************** 新建 ***************** //
const newVolumeDialog = defineAsyncComponent(() => import('./EditVolumeDialog.vue'))
...
const handleDelete = (index: number, row) => {
  ...
  ElMessageBox.confirm(
    i18n.global.t('volume.confirm_delete_volume', { 'title': row.title }),
    currentBook.title,
    {
    ...
    .then(() => {
      BookService.deleteVolume({ bookid: bookid, id: row.id })
        .then(() => {
          ...
          ElMessage.success(i18n.global.t('volume.volume_deleted_ok'))
        })
        .catch(err => {
          ElMessage.error('VOLDERR: ' + i18n.global.t('common.deleted_error', { 'err': (err.messge || err.msg) }))
        })
    })
    .catch(() => {
      // 不删除
