<template>
  ...
  <el-row>
    <el-col :span="24" style="text-align: right;padding-bottom: 10px;">
      <el-button type="primary" icon="IconCustomAddBook" @click="handleNewVolDialog()" class="btn-search">{{ $t('volume.pagetitle_add_volume') }}</el-button>
    </el-col>
  </el-row>
  <el-table v-loading="fsLoading" :header-row-class-name="tableHeadClass" :data="tableData" :highlight-current-row="true" stripe border>
    ...
    <el-table-column fixed="right" :label="$t('common.operation')" width="100">
      <template #default="scope">
        <el-button-group>
          ...
          <el-button text size="small" icon="IconoirTrash" @click="handleDelete(scope.$index, scope.row)" />
        </el-button-group>
      </template>
    </el-table-column>
  </el-table>
  <Suspense v-if="newVolumeVisible">
    <template #default>
      <newVolumeDialog :volume="currentVol" @hide="newVolumeVisible = false" @update="afterVolUpdated" @close-loading="closeLoading" />
    </template>
  </Suspense>
</template>
    ...
    ElMessageBox.confirm(
      ...
      .then(() => {
        BookService.deleteVolume({ bookid: bookid, id: row.id })
          ...
          .catch(err => {
            ElMessage.error('VOLDERR: ' + _t('common.deleted_error', { 'err': (err.messge || err.msg) }))
          })
      })
      .catch(() => {
        // 不删除
  ...
  const getVolumes = () => {
    BookService.getVolumes({ bookid: bookid })
      .then(resp => {
        ...
        d.items.forEach((item) => { if (item.cover) { coverPreviewList.push(item.cover) } })
