<template>
  ...
  <h4>{{ currentBook.title }} <small v-if="currentBook.title != currentChapter.volumeTitle">{{ currentChapter.volumeTitle }}</small></h4>
  <el-button circle type="warning" size="large" :icon="flippingIcon" v-if="showFlipping" style="top:50px;position:fixed;" />
  <el-row @dblclick="onDoubleClick">
    <el-col :xs="0" :sm="2" :md="4"></el-col>
    <el-col :xs="24" :sm="20" :md="16">
      <div class="content" v-html="nl2br(currentChapter.content)" @touchstart="onTouchStart" @touchmove="onTouchMove" @touchend="onTouchEnd" />
    </el-col>
  </el-row>
  <el-divider />
  <el-row style="padding-top: 20px;">
    <el-col :span="24">
      <el-button icon="IconoirNavArrowLeft" type="primary" @click="handleFlip('right')" v-if="prevChap.id">{{ $t('chapter.previous') }}</el-button>
      <el-button icon="IconoirPageEdit" type="primary" @click="handleEdit(currentChapter.id)" v-if="currentChapter.id" />
      <el-button type="primary" @click="handleFlip('left')" v-if="nextChap.id">{{ $t('chapter.next') }} <el-icon class="ep-icon--right">
...
import {
  nl2br, intval,
  handleDoubleClick, handleTouchEnd, handleTouchMove, handleTouchStart,
  flippingIcon, showFlipping
} from '~/utils'
...
const handleEdit = (chapterid: number) => {
  router.push({ name: 'editchapter', params: { 'bookid': bookid.value, 'chapterid': chapterid } })
}
...
watch(KeyArrowLeft, (v) => { if (v && !gostore.dialogPopup) { handleFlip('right') } })
watch(KeyArrowRight, (v) => { if (v && !gostore.dialogPopup) { handleFlip('left') } })
