<template>
  <el-form :inline="true" :model="searchForm" ref="searchFormRef" :rules="rules" @submit.prevent @keyup.enter="submitForm(searchFormRef)" size="default">
    <el-form-item class="search-area" prop="q">
      <el-input size="large" v-model.trim="searchForm.q" :placeholder="$t('search.content_input_placeholder')">
        <template #append>
          <el-button icon="IconoirSearch" @click="submitForm(searchFormRef)" class="btn-search" />
        </template>
      </el-input>
      <el-checkbox v-if="showSearchInBook" v-model="inbook" :label="searchInBookLabel" size="large" />
    </el-form-item>
  </el-form>
  <el-divider />
...
<script lang="ts" setup>
  ...
  const colorWords = (words: string) => {
    return words.replaceAll(searchForm.q, '<span style="color:#ff7866;">' + searchForm.q + '</span>').replaceAll('　', '')
  }
