<template>
  <el-dialog :model-value="true" top="8vh" close-icon="IconoirXmarkCircle" :close-on-press-escape="false" :before-close="handleClose" @opened="closeLoading" :title="$t('search.search')"
    ...
    <el-form :model="searchForm" ref="searchFormRef" :rules="rules" size="large" class="search-form" @submit.prevent @keyup.enter="submitForm(searchFormRef)">
      <el-form-item prop="q">
        <el-input clearable v-model.trim="searchForm.q" ref="queryRef" :placeholder="$t('search.dialog_input_placeholder')">
          <template #append>
...
<script lang="ts" setup>
  ...
  const changeSearchMode = () => {
    showSearchInBook.value = searchForm.sm === 'content' && (gostore.book?.id ? true : false)
  }
  ...
  onMounted(() => {
    nextTick(() => {
      const queryInput = queryRef.value as HTMLInputElement
      queryInput.focus()
    })
  })
</script>
