<template>
  <div class="flex items-center pageheader-extra">
    <div style="margin-right: 50px;" v-if="showSearch">
      <el-form :model="searchForm" :rules="{}" @submit.prevent @keyup.enter="submitForm(searchFormRef)">
        <el-input v-model.trim="gostore.query" clearable :placeholder="$t('search.header_placeholder')">
          <template #append>
    ...
    <div>
      <el-button circle size="small" icon="IconoirSearch" @click="handleSearchDialog()" :title="searchBookTitle()" />
      <el-button circle size="small" icon="IconCustomAddBook" @click="handleNewBookDialog()" :title="newBookTitle()" />
      <el-dropdown trigger="click" @command="handleCommand" class="dropdown">
        <el-button circle size="small" icon="IconoirNavArrowDown" />
        <template #dropdown>
          <el-dropdown-menu>
            <el-dropdown-item icon="IconoirPeopleTag" command="user">{{ $t('common.welcome') }}, {{ mobile }}</el-dropdown-item>
            <el-dropdown-item icon="IconoirKey" command="pwd">{{ $t('config.edit_pwd') }}</el-dropdown-item>
            <el-dropdown-item :icon="darkIcon()" command="color" divided>{{ darkMenuTitle() }}</el-dropdown-item>
            <el-dropdown-item :icon="localeIcon('zhCN')" command="locale_zhcn" divided>{{ $t('config.locale_zhcn') }}</el-dropdown-item>
            <el-dropdown-item :icon="localeIcon('zhTW')" command="locale_zhtw">{{ $t('config.locale_zhtw') }}</el-dropdown-item>
            <el-dropdown-item :icon="localeIcon('en')" command="locale_en">{{ $t('config.locale_en') }}</el-dropdown-item>
            <el-dropdown-item icon="IconoirSettings" command="sysp" divided>{{ $t('config.system_preferences') }}</el-dropdown-item>
            <el-dropdown-item icon="IconoirGroup" command="author" divided>{{ $t('config.author_mgmt') }}</el-dropdown-item>
            <el-dropdown-item icon="IconCustomExit" command="exit" divided>{{ $t('config.exit') }}</el-dropdown-item>
          </el-dropdown-menu>
        </template>
      </el-dropdown>
    </div>
  ...
  <Suspense v-if="bookDialogVisible">
    <template #default>
      <newBookDialog @hide="bookDialogVisible = false" @close-loading="closeLoading" :dgwidth="dgWidth" :book="newBook" @update="afterBookUpdated" />
    </template>
  </Suspense>
...
<script lang="ts" setup>
  ...
  import { dialogWidth, defaultDialogWidth, newbookShortcut, searchShortcut } from '~/utils';
  import { toggleDark, isDark } from '~/composables';
  import { IconoirSunLight, IconoirHalfMoon, IconoirTranslate, IconoirCheck } from '~/composables';
  ...
  watch(meta_k, (v) => { if (v) { handleSearchDialog() } })
  watch(meta_b, (v) => { if (v) { handleNewBookDialog() } })
