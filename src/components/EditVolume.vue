<template>
  <el-dialog :model-value="true" top="8vh" :close-on-click-modal="false" :close-on-press-escape="false"
    ...
    <el-form :model="volForm" ref="ruleFormRef" label-position="top" label-width="auto" :rules="rules" size="large">
      <el-form-item label="卷名" prop="title">
        <el-input v-model="volForm.title" autocomplete="off" />
      </el-form-item>
...
import { Volume } from '~/models/book'
...
const update = () => {
  BookService.updateVolume(volForm).then(
    resp => {
      ElMessage.success('图书卷（' + volForm.title + '）信息保存成功。')
