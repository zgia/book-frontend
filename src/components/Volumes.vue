<template>
  ...
  <el-table v-loading="fsLoading" :header-row-class-name="tableHeadClass" :data="tableData" :highlight-current-row="true"
    stripe border>
    <el-table-column prop="title" label="卷名" width="200" />
  ...
  <Suspense v-if="showCreationDialog">
    <template #default>
      <creationDialog :volume="currentVol" @hide="showCreationDialog = false" @update="afterUpdated"
...
const handleEdit = (index: number, row: Volume) => {
  openCreationDialog(index, row)
}

const openCreationDialog = (index: number, row: Volume) => {
  openLoading()
