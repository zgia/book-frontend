<template>
  <el-table v-loading="fsLoading" :header-row-class-name="tableHeadClass" :data="authorData" :highlight-current-row="true" tripe border>
    <el-table-column :label="$t('author.name')" width="200">
      <template #default="scope">
        <router-link :to="authorUrl(scope.row)" class="ep-link author-url">{{ scope.row.name }} </router-link>
      </template>
    </el-table-column>
    ...
    <el-table-column :label="$t('author.books')" v-if="showAuthor">
      <template #default="scope">
        <router-link :to="authorUrl(scope.row)" class="ep-link author-url">{{ scope.row.books }}</router-link>
      </template>
    </el-table-column>
    <el-table-column :label="$t('common.operation')" width="120">
      <template #default="scope">
        <el-button text size="small" icon="IconoirEdit" @click="handleEdit(scope.$index, scope.row)">{{ $t('common.btn_edit') }}</el-button>
        <br />
        <el-button text size="small" type="danger" icon="IconoirTrash" @click="handleDelete(scope.$index, scope.row)">{{ $t('common.btn_delete') }}</el-button>
      </template>
    </el-table-column>
  </el-table>
  ...
  <Suspense v-if="editAuthorVisible">
    <template #default>
      <editAuthorDialog v-model="current.author" :dgwidth="dgWidth" @hide="editAuthorVisible = false" @update="afterAuthorUpdated" @close-loading="closeLoading" />
    </template>
  </Suspense>
  <el-backtop />
</template>
<script lang="ts" setup>
  ...
  // ***************** 图书信息 ***************** //
  const authorUrl = (row: Author) => {
    return `/?sm=author&q=${row.name}`
  }
  ...
  // ***************** 编辑 ***************** //
  const editAuthorDialog = defineAsyncComponent(() => import('./EditAuthorDialog.vue'))
      ...
      .then(() => {
        AuthorService.deleteAuthor({ id: row.id })
          .then(() => {
            ...
            ElMessage.success(_t('author.author_deleted_ok', { 'title': row.name }))
          })
          .catch(err => {
            ElMessage.error('AUTHORERR: ' + _t('common.deleted_error', { 'err': (err.messge || err.msg) }))
          })
      })
      .catch(() => {
        // 不删除
  ...
  const handleChangePage = (p: number) => {
    params.p = p
    router.push({ name: 'author', query: params })
    getAuthors()
  }
  ...
  const getAuthors = () => {
    ...
    AuthorService.list({ p: params.p, words: params.q })
      .then(resp => {
        const d = resp.data
