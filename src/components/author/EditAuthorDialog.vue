<template>
  <el-dialog :model-value="true" top="8vh" close-icon="IconoirXmarkCircle" :width="props.dgwidth" :close-on-press-escape="false" :before-close="handleClose" @opened="closeLoading" :title="pageTitle">
    <el-form :inline="true" :model="authorForm" ref="ruleFormRef" label-position="top" label-width="auto" :rules="rules" size="large">
      <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24">
        <el-form-item :label="$t('author.name')" prop="name">
          <el-input v-model.trim="authorForm.name" ref="titleRef" clearable autocomplete="off" :placeholder="$t('author.name_placeholder')" />
        </el-form-item>
      </el-col>
      <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24">
        <el-form-item :label="$t('author.former_name')">
          <el-input v-model.trim="authorForm.former_name" clearable autocomplete="off" :placeholder="$t('author.former_name_placeholder')" />
        </el-form-item>
      </el-col>
    </el-form>
    <template #footer>
...
<script lang="ts" setup>
  ...
  const pageTitle = vForm.value.id ? _t('author.dialog_edit_author') : _t('author.dialog_add_author')
  ...
  const rules = reactive<FormRules>({
    name: [{ required: true, message: _t('author.dialog_author_name_is_empty'), trigger: 'blur' }]
  })
  ...
  const update = () => {
    AuthorService.updateAuthor(authorForm)
      ...
      .catch(err => {
        ElMessage.error('AUTHORCERR: ' + _t('author.saved_error', { 'err': (err.messge || err.msg) }))
      })
  }
