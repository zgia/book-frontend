...
import {
  ...
  IconoirNavArrowRight, IconoirNavArrowLeft,
} from '~/composables'
...
export const handleTouchStart = (e: TouchEvent) => {
  ...
  if (tapTimes.start && !tapTimes.end && dn - tapTimes.start > tapTimeout) {
    ...
    // 表示第2次点击
    if (tapTimes.start) { tapTimes.end = dn }
    // 表示第1次点击
    if (!tapTimes.start) { tapTimes.start = dn }

    if (tapCoords.startX) { tapCoords.endX = tapX; tapCoords.endY = tapY }
    if (!tapCoords.startX) { tapCoords.startX = tapX; tapCoords.startY = tapY }
  }
}
...
export const handleTouchEnd = (onFlip: (d: string) => void) => {
  ...
  // 双击应该在同一位置
  if (tapTimes.end - tapTimes.start > tapTimeout || Math.abs(tapCoords.startX - tapCoords.endX) > tapDistance || Math.abs(tapCoords.startY - tapCoords.endY) > tapDistance) {
    clearTap()
    return
  }
...
export const handleDoubleClick = (e: MouseEvent, onFlip: (d: string) => void) => {
